...
# Set precision to 16 for mixed precision training
# Use 'devices' instead of 'gpus'
# trainer = Trainer(precision=16, devices=2, strategy='ddp')  # 'ddp' for distributed data parallel
...
def validate(fabric: L.Fabric, model: Model, val_dataloader: DataLoader, epoch: int = 0, log_file_path: str = None):
    model.eval()
    ious = AverageMeter()
    f1_scores = AverageMeter()
    val_losses = AverageMeter()  # Keep track of validation loss
    val_focal_losses = AverageMeter()  # Keep track of validation focal loss
    val_dice_losses = AverageMeter()  # Keep track of validation dice loss
    focal_loss = FocalLoss()
    dice_loss = DiceLoss()

    mismatch_count = 0  # Counter for mismatched instances in validation

    # Open the log file for writing mismatched image paths (if log_file_path is provided)
    log_file = None
    if log_file_path:
        log_file = open(log_file_path, 'a')
        log_file.write(f"\nValidation Mismatched Image Paths and IDs (Epoch {epoch}):\n")  # Header for validation mismatches

    with torch.no_grad():
        for iter, data in enumerate(val_dataloader):
            images, bboxes, gt_masks, image_paths, image_ids = data  # Include image paths and IDs
            ...
            # Check for dimension mismatches and log them
            skip_batch = False
            for i, (pred_mask, gt_mask) in enumerate(zip(pred_masks, gt_masks)):
                if pred_mask.shape != gt_mask.shape:
                    mismatch_count += 1
                    skip_batch = True
                    fabric.print(f"Validation: Skipping batch due to mismatch: pred_mask shape {pred_mask.shape}, gt_mask shape {gt_mask.shape}")

                    # Write the image path, image ID, and shapes to the log file
                    if log_file:
                        log_file.write(f"Iteration: {iter},  "
                                       f"Predicted Shape: {pred_mask.shape}, Ground Truth Shape: {gt_mask.shape}\n")
                    break  # Exit the loop for this batch

            if skip_batch:
                continue  # Skip the batch entirely
            ...
            fabric.print(
                ...
                f'Val Loss: [{val_losses.avg:.4f}] -- Val Focal Loss: [{val_focal_losses.avg:.4f}] -- Val Dice Loss: [{val_dice_losses.avg:.4f}]'
            )

    # Log total mismatched instances for validation
    if log_file:
        log_file.write(f'\nTotal Mismatched Instances in Validation: {mismatch_count}\n')
        log_file.close()  # Close the log file after validation
...
#                 print(f"Pred mask: {pred_mask.shape}, dtype: {pred_mask.dtype}")
#                 print(f"GT mask: {gt_mask.shape}, dtype: {gt_mask.dtype}")
...
def train_sam(
    ...
    mismatch_count = 0  # Counter for mismatched instances

    # Open the log file for writing mismatched image paths
    with open(log_file_path, 'w') as log_file:
        log_file.write("Mismatched Image Paths and IDs:\n")  # Header for the log
        ...
        for epoch in range(1, cfg.num_epochs):
            ...
            for iter, data in enumerate(train_dataloader):
                optimizer.zero_grad()
                data_time.update(time.time() - end)
                images, bboxes, gt_masks = data  # Now includes image_ids from annotation.json
                batch_size = images.size(0)
                pred_masks, iou_predictions = model(images, bboxes)
                num_masks = sum(len(pred_mask) for pred_mask in pred_masks)
                loss_focal = torch.tensor(0., device=fabric.device)
                ...
                # Check for dimension mismatch and log if they don't match
                skip_batch = False
                for i, (pred_mask, gt_mask) in enumerate(zip(pred_masks, gt_masks)):
                    if pred_mask.shape != gt_mask.shape:
                        mismatch_count += 1
                        skip_batch = True
                        fabric.print(f"Skipping batch due to mismatch: pred_mask shape {pred_mask.shape}, gt_mask shape {gt_mask.shape}")
                        
                        # Write the image path, image ID, and shapes to the log file
                        log_file.write(f"Epoch: {epoch}, Iteration: {iter}, "
                                       f"Predicted Shape: {pred_mask.shape}, Ground Truth Shape: {gt_mask.shape}\n")
                        break  # Exit the loop for this batch
        ...
        # Log the total mismatch count
        fabric.print(f'Total Mismatched Instances: {mismatch_count}')
        with open(log_file_path, 'a') as log_file:
            log_file.write(f'\nTotal Mismatched Instances: {mismatch_count}\n')
...
def main(cfg: Box) -> None:
    ...
    # Pass log_file_path to train_sam function
    train_sam(cfg, fabric, model, optimizer, scheduler, train_data, val_data, log_file_path)
