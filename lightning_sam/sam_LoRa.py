...
class LoRA_Sam(nn.Module):
    ...
    def forward(self, batched_input, multimask_output,image_size=1024):
        print(f"batched_input: {batched_input.shape}, type: {type(batched_input)}")
        print(f"multimask_output type: {type(multimask_output)}")
        print(f"image_size: {image_size}, type: {type(image_size)}")
