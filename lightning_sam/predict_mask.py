...
def draw_binary_mask(image, masks, alpha=0.4):
    ...
    # Ensure masks are on CPU and convert to numpy array
...
def visualize(cfg: Box):
    ...
    for image_id in tqdm(dataset.image_ids):
        ...
        image_output = draw_binary_mask(image, masks.squeeze(1))

        # Add debug statement to check the size and type of the output mask
        print(f"Saving to: {image_output_path}, Mask shape: {image_output.shape}, Mask dtype: {image_output.dtype}")

        if image_output.ndim == 2 and image_output.shape == image.shape[:2]:
            cv2.imwrite(image_output_path, image_output)
        else:
            print(f"Skipping saving due to invalid mask shape or dimensions: {image_output.shape}")
